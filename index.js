...
/******** OWNER NUMBER**********/
const ownerNumber = ["558897187831@s.whatsapp.net","558897187831@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `menos 1 @${num.split('@')[0]} kkkj vai memo, n faz falta`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           return false
			...
			switch(command) { 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!tex) return client.sendMessage(from, 'INSIRA O URL / TEXTO PARA SER QR', text, {quoted: mek})
            ...
            case 'bitly':
               ...
               case 'help': 
         	   ...
         	   case 'owner':
         	   case 'creator':
                  client.sendMessage(from, {displayname: "Lyno", vcard: vcard}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, 'Este é o número do meu proprietário > _ <, n trava man pfv',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
				bo = args[0]
...
• Limit: ${limitawal-updated.limit}`
            ...
            break
                ...
                addKoinUser(558897187831@s.whatsapp.net', fee)
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
 			 ...
 			 //  reply('p\n' + util.format({fontPath, inputPath, outputPath, tgl, hari, textnulis}))
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return client.sendMessage(from, 'coloca o username', text, {quoted: mek})
						...
						console.log(`Error :`, color(e,'red'))
						reply('coloca o user o teco teco direito')
					}
					await limitAdd(sender)
				break
				//group feature 
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`o mano @${mentioned[0].split('@')[0]} conseguiu perder o adm, deêm os parabéns pra ele 🤣🤣`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('maque a pessoa p virar adm')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`parabens seu bosta @${mentioned[0].split('@')[0]} *você se tornou o adm do grupo* (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `vai de ralo ksks 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`inutil, botando p vazar... @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `Lista dos cornos *${groupMetadata.subject}*\ntotal de gados q tem no rebanho : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ ATIVADO ❭ ️Desabilite o recurso de boas-vindas ️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ DESATIVADO ❭ ️Habilite o recurso de boas-vindas ')
					} else {
						reply(ind.satukos())
					}
					break 
					...
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('ja ativo !!!')
						samih.push(from)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
						reply('❬ ATIVADO ❭ Desative o recurso simi️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
						reply('❬ DESATIVADO ❭ Habilite o recurso simi )
					} else {
						reply(ind.satukos())
					}
					break
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ Ativado ❭ Desative o recurso nsfw ')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ Desativado ❭ ative o recurso nsfw ️')
					} else {
						reply(ind.satukos())
					}
				break
				...
				case 'clone':
					...
					if (args.length < 1) return reply(' *marque quem vc quer q eu use a ft de perfil* ')
				...
				case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('GRUPO DE PERFIL ALTERADO COM SUCESSO')
					break				
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
				if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'flw troxa').then(() => client.groupLeave(from))
					break 
				case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
					if (args.length < 1) return reply(' oq vc quer botar na tm? ')
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
                    prefix = args[0]
                    reply(`mudei o prefixo para  ${prefix} 😐✌ `)
					break 
				case 'setmemlimit':
					if (args.length < 1) return
					if (!isOwner) return reply(ind.ownerb())
